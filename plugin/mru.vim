...
" MRU_Compare
" comparation function for the keys according to the index numbers
" they get in s:MRU_Files
function! s:MRU_Compare(k1, k2)
   let i1= s:MRU_Files[a:k1]
   let i2= s:MRU_Files[a:k2]
   return i1==i2 ? 0 : i1 > i2 ? -1 : 1
endfunction

" MRU_Files_As_List
" Retrieve all mru-files as a list
function! s:MRU_Files_As_List()
   let files= []
   let n=0
   for fl in sort(keys(s:MRU_Files),"s:MRU_Compare")
      call add(files, fl)   
      let n+= 1
      if n >= g:MRU_Max_Entries
         break
      endif
   endfor
   return files
...
" MRU_Files_As_String
" return the list for MRU-Files as a newline separated string
function! s:MRU_Files_As_String()
   return join(s:MRU_Files_As_List(),"\n")
endfunction
...
function! s:MRU_AddFile(filename)
    ...
    " Under windows handle all files as lowercase
    if has("win32")
       let fname= tolower(fname)
    endif
    ...
    let s:MRU_Files[fname]= s:MRU_Max
    let s:MRU_Max+=1
...
function! s:MRU_LoadList()
    ...
    if filereadable(g:MRU_File)
       let n=1
       for line in readfile(g:MRU_File)
         if filereadable(line)
            let s:MRU_Files[line]= n
            let n+= 1
         endif
       endfor
       let s:MRU_Max= n
...
function! s:MRU_SaveList()
    ...
    call writefile(reverse(s:MRU_Files_As_List()), g:MRU_File)
...
function! s:MRU_Open_Window()
    ...
    "silent! 0put =s:MRU_Files_As_String()
    silent! call append(0, s:MRU_Files_As_List())
...
function! s:MRU_Edit_File_Cmd(fspec)
    ...
    let mru_list= s:MRU_Files_As_List()
    let cnt= len(mru_list)
    ...
    if a:fspec =~ '\d\+'
        ...
        if fnum > cnt
            echohl WarningMsg
            echo 'Number of entries in MRU list = ' .
                        \ cnt
            echohl None
            return
        endif
        
        let fname= mru_list[fnum-1]
        call s:MRU_Edit_File(fname)
    else
        " Locate the file name matching the supplied partial name
        let fname = ''
        let fname_cnt = 0
        for one_fname in mru_list
            if stridx(one_fname, a:fspec)>=0
                let fname = one_fname
                let fname_cnt+= 1
            endif
        endfor
...
function! s:MRU_Refresh_Menu()
    ...
    for fname in s:MRU_Files_As_List()
...
augroup mru
   ...
   autocmd VimLeave * call s:MRU_SaveList()
augroup end

" Command to open the MRU window
command! -nargs=0 MRUload call s:MRU_LoadList()
command! -nargs=0 MRUsave call s:MRU_SaveList()
